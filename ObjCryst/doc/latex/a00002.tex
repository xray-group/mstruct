\begin{DoxyVerb}This project is still in active development, even if I left the Lab of Crystallography
\end{DoxyVerb}
 in Geneva to the E\-S\-R\-F. If you encounter some problem with the library, please contact me at {\tt vincefn@users.\-sourceforge.\-net} and I'll try to help.
...
You can download the \doxyref{Obj\-Cryst}{p.}{a00178} library from the {\tt Obj\-Cryst project page} on Source\-Forge, or from the mirrors\-:
\begin{DoxyItemize}
\item {\tt C\-C\-P14 (U\-K)}
\item {\tt C\-C\-P14 (Ca)}
...
You will need to download the \doxyref{Obj\-Cryst}{p.}{a00178} tar.\-bz2 (.zip for windows), as well as the 'external libraries' tar.\-gz (.zip) archives (the latter include libraries which are used by this project\-: currently atominfo,sglite and newmat). Simply uncompress both archives in the same directory. This will create the atominfo, sglite and newmat directories, as well as the \doxyref{Obj\-Cryst}{p.}{a00178} directory. This documentation is in Obj\-Cryst/doc/html/
...
The cutting-\/edge version of the library can always be obtained through its C\-V\-S repository. Under linux, to download the first time the Obj\-Cryst++ directory, first login in the C\-V\-S repository (give a blank password when asked) and then download the \doxyref{Obj\-Cryst}{p.}{a00178} source\-:
\begin{DoxyCode}
...
*      cvs update 
\end{DoxyCode}
 (with a blank password) at the root of the \doxyref{Obj\-Cryst}{p.}{a00178} directory.
...
Under linux (resp. windows), copy the rules-\/gnu.\-mak (resp. rules-\/bc32.\-mak) to rules.\-mak at the root of the \doxyref{Obj\-Cryst}{p.}{a00178} directory. You can edit this file to check the paths and (for Windows) some compiling options.
\end{DoxyParagraph}
...
\end{DoxyParagraph}
(use wx\-Cryst=0 if you do not want the Graphical interface, and debug=1 for faster compiling and debugging messages. Not that anytime you change one of these options, you need to clean up all directories\-: type \char`\"{}make -\/f gnu.\-mak clean\char`\"{} at the root of the \doxyref{Obj\-Cryst}{p.}{a00178} directory)
